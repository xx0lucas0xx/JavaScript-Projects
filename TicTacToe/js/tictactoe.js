...
function placeXOrO(squareNumber) {
    ...
    //The .some() method is used to check the element of the selectSquare array
    //to see if it contains the square number clicker on.
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if(activePlayer === 'X') {
            ...
            select.style.backgroundImage = 'url("images/Walter.jpg")';
            //Active player may only be 'Walter' or 'Bingus' so, if not 'X' it must be 'O'
        } else {
            //If activePlayer is equal to 'animationLoop', the .png is placed in HTML
            select.style.backgroundImage = 'url("images/Bingus.jpg")';
        }
...
function restGame() {
    //This for loop iterates through each HTML square elment.
    for (let i = 0; i < 9; i++) {
